...
export default function EntertainActivityModal({
  ...
  useEffect(() => {
    ...
    if (editingCard) {
      ...
      // hỗ trợ cả data.time, data.startTime, editingCard.startTime
      const loadedStart =
  ...
  const footerLeft = (
    <div className="hidden sm:flex flex-col text-[11px] text-slate-500 dark:text-slate-400">
      <span>
        {placeName
          ? `Vui chơi: ${placeName}`
          : "Điền địa điểm vui chơi để lưu hoạt động."}
      </span>
      {address && (
        <span>
  ...
  return (
    ...
    >
      ...
      {/* CHI PHÍ VUI CHƠI + NGÂN SÁCH + PHÁT SINH + THỰC TẾ */}
      <section className="space-y-3">
        ...
        <div className={sectionCard + " space-y-4"}>
          ...
          <div>
            <div className="flex items-center justify-between gap-2 mb-1">
              <label className="text-xs font-medium text-slate-600 dark:text-slate-300">
                Chi phí phát sinh (thuê đồ, gửi xe, ăn uống... trong lúc chơi)
              </label>
              <button
                type="button"
              ...
              {extraItems.map((item, idx) => (
                <div key={idx} className="flex items-center gap-2">
                  <input
                    ...
                    placeholder="Ví dụ: thuê phao, gửi xe..."
                    className={`${inputBase} flex-1`}
                  />
                  <div className="flex items-center gap-2">
                    <FaMoneyBillWave className="text-emerald-500" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-[11px] text-slate-500 dark:text-slate-400">
                Nếu để trống, hệ thống sẽ dùng <b>chi phí vé + phát sinh</b> để chia
                tiền.
              </p>
            </div>
