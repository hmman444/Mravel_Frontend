...
const ACTIVITY_META = {
  STAY:        { label: "Lưu trú",    color: "text-violet-700 bg-violet-100 dark:text-violet-200 dark:bg-violet-900/40", icon: "🏨" },
  FOOD:        { label: "Ẩm thực",    color: "text-orange-700 bg-orange-100 dark:text-orange-200 dark:bg-orange-900/40", icon: "🍜" },
  SIGHTSEEING: { label: "Tham quan",  color: "text-emerald-700 bg-emerald-100 dark:text-emerald-200 dark:bg-emerald-900/40", icon: "📍" },
  TRANSPORT:   { label: "Di chuyển",  color: "text-sky-700 bg-sky-100 dark:text-sky-200 dark:bg-sky-900/40", icon: "🚗" },
  ENTERTAIN:   { label: "Giải trí",   color: "text-pink-700 bg-pink-100 dark:text-pink-200 dark:bg-pink-900/40", icon: "🎉" },
  EVENT:       { label: "Sự kiện",    color: "text-amber-700 bg-amber-100 dark:text-amber-200 dark:bg-amber-900/40", icon: "🎪" },
  SHOPPING:    { label: "Mua sắm",    color: "text-yellow-700 bg-yellow-100 dark:text-yellow-200 dark:bg-yellow-900/40", icon: "🛍️" },
  CINEMA:      { label: "Rạp phim",   color: "text-fuchsia-700 bg-fuchsia-100 dark:text-fuchsia-200 dark:bg-fuchsia-900/40", icon: "🎬" },
  OTHER:       { label: "Khác",       color: "text-slate-700 bg-slate-200 dark:text-slate-200 dark:bg-slate-700/60", icon: "•" },
};
    ...
    <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800/60">
      ...
      <div className="border-b border-slate-200 bg-secondary/10 px-5 py-4 dark:border-slate-700 dark:bg-slate-800/80">
        <div className="font-heading text-[15px] font-bold text-slate-800 dark:text-slate-100">
          {draft.summary || `Lịch trình tại ${draft.destination}`}
        </div>
        <div className="mt-2 grid grid-cols-2 gap-x-4 gap-y-1.5 text-xs text-slate-600 dark:text-slate-300 sm:grid-cols-4">
          <Meta label="Điểm đến" value={draft.destination} />
          <Meta label="Khoảng" value={`${draft.start_date} → ${draft.end_date}`} />
          <Meta label="Người" value={`${draft.travelers || 1} người`} />
          <Meta label="Tổng chi phí" value={formatVnd(draft.estimated_total_cost_vnd)} />
        </div>
        <div className="mt-2 text-[11px] text-slate-500 dark:text-slate-400">
          {draft.days?.length || 0} ngày · {totalActivities} hoạt động
        </div>
