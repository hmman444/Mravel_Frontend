...
export default function Navbar({ fixedWhite = false }) {
  const [scrolled, setScrolled] = useState(false);

  useEffect(() => {
    if (fixedWhite) return;
    const handleScroll = () => setScrolled(window.scrollY > 50);
    window.addEventListener("scroll", handleScroll);
    return () => window.removeEventListener("scroll", handleScroll);
  }, [fixedWhite]);

  const isActive = fixedWhite || scrolled;

  return (
    <header
      className={`fixed top-0 w-full z-30 transition-all duration-300 ${
        isActive ? "bg-white shadow dark:bg-gray-900" : "bg-transparent"
      }`}
    >
      <div className="max-w-7xl mx-auto px-6 py-3 flex justify-between items-center">
        ...
        <div className="flex gap-3">
          ...
          <a
            href="/register"
            className="px-4 py-2 rounded-lg text-sm bg-primary text-white hover:bg-primaryHover dark:bg-secondary dark:text-gray-900"
          >
            Đăng ký
          </a>
          <DarkModeToggle />
